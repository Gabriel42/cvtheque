...
export default function App() {
  ...
  const filtered = activeCategory ? candidates.filter(c => {
    if (c.category !== activeCategory) return false;
    if (!search) return true;
    const q = search.toLowerCase();
    return c.name.toLowerCase().includes(q) || c.title.toLowerCase().includes(q) || c.skills.some(s => s.toLowerCase().includes(q)) || c.city.toLowerCase().includes(q) || c.summary.toLowerCase().includes(q);
  }) : [];
          ...
          {!activeCategory ? (
            <div>
              ...
              <div style={{ position: "relative", marginBottom: "1.5rem" }}>
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un profil par nom, poste, compétence, ville…"
                  style={{ width: "100%", boxSizing: "border-box", padding: "10px 36px 10px 36px", borderRadius: 10, fontSize: 14, border: `1px solid ${search ? "#4F46E5" : "var(--color-border-secondary)"}`, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", transition: "border-color 0.15s" }} />
                {search && (
                  <button onClick={() => setSearch("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 2, color: "var(--color-text-tertiary)" }}>
              ...
              {search ? (() => {
                const q = search.toLowerCase();
                const results = candidates.filter(c => c.name.toLowerCase().includes(q) || c.title.toLowerCase().includes(q) || c.skills.some(s => s.toLowerCase().includes(q)) || c.city.toLowerCase().includes(q) || c.summary.toLowerCase().includes(q));
                return results.length === 0 ? (
                  ...
                  <>
                    <p style={{ fontSize: 13, color: "var(--color-text-secondary)", margin: "0 0 1rem" }}>{results.length} résultat{results.length > 1 ? "s" : ""} pour « {search} »</p>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
                      {results.map(c => { const cat = CATEGORIES.find(cat => cat.id === c.category); return <CandidateCard key={c.id} c={c} onClick={setSelected} catColor={cat?.color || "#4F46E5"} />; })}
                    </div>
                  </>
                );
              })() : (
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: "1rem", flexWrap: "wrap" }}>
                <button onClick={() => { setActiveCategory(null); setSearch(""); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 10, border: "0.5px solid var(--color-border-secondary)", background: "transparent", color: "var(--color-text-secondary)", fontSize: 13, cursor: "pointer" }}>
              ...
              <div style={{ position: "relative", marginBottom: "1.5rem" }}>
                <i className="ti ti-search" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", fontSize: 15, color: "var(--color-text-tertiary)", pointerEvents: "none" }} aria-hidden="true" />
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher par nom, poste, compétence, ville…"
                  style={{ paddingLeft: 36, borderRadius: 10, fontSize: 14, width: "100%", boxSizing: "border-box", padding: "10px 12px 10px 36px", border: `1px solid ${search ? activeCat.color : "var(--color-border-secondary)"}`, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", transition: "border-color 0.15s" }} />
                {search && (
                  <button onClick={() => setSearch("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 2, color: "var(--color-text-tertiary)" }}>
                    <i className="ti ti-x" style={{ fontSize: 14 }} aria-hidden="true" />
                  </button>
